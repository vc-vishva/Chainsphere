...
export class AuthService {
  ...
  //*******************************************************************Admin services*******************************************************************/
  /**
   * Description - Admin signup service
   * @param createUserDto CreateUserDto
   * @returns Common success | error response
   */
  async signup(createAdminDto: CreateAdminDto, validApiKey: boolean): OnlyMessageResponse {
    if (!validApiKey) return ResponseHandler.error([], errorMessages.UNAUTHORIZED_REQUEST, HttpStatus.UNAUTHORIZED);

    const adminUserWithEmail = await this.userService.getUserJson({
    ...
    createAdminDto['userType'] = 'Admin';
    const merchant = await this.userService.createAdmin(createAdminDto);
    // send notification email the new admin on boarding our system
    await this.commonMailService
      .sendEmailWithTemplate(
        {
          organizationName: `${createAdminDto.merchantName}`,
          organizationEmail: createAdminDto.email,
          date: moment(new Date()).startOf('day').format('MMMM D, YYYY'),
          donorProfileUrl: undefined,
        },
        EmailTemplatePath.ADMIN_JOIN_COMMUNITY,
        {},
        {
          to: [`${this.configService.get('SUPER_ADMIN_EMAIL')}`, `${this.configService.get('SUPER_ADMIN_EMAIL_2')}`],
          cc: [`${this.configService.get('CC_EMAIL_1')}`, `${this.configService.get('CC_EMAIL_2')}`],
          subject: Defaults.NEW_ADMIN_JOIN_COMMUNITY_SUBJECT,
        },
      )
      .then(() => {
        return true;
  ...
  async login(loginDto: UserLoginDto): LoginResponse {
    ...
    if (!admin.emailVerified) throw new NotAcceptableException(errorMessages.EMAIL_NOT_VERIFIED);
    ...
    return ResponseHandler.success(
      {
        ...
        appliedForMerchantApp: admin.appliedForMerchantApp,
        email: admin.email,
        merchantName: admin.merchantName,
        paymentGateway: admin.paymentGateway,
        accessToken,
        isProfileLive: admin.isProfileLive,
        availableForCelebration: admin.availableForCelebration,
      },
  ...
  /**
   * Description - Admin Forgot password service
   * @param forgotPasswordDto SendEmailDto
   * @returns Common success | error response
   */
  async adminForgotPassword(adminForgotPasswordDto: AdminForgotPasswordDto): OnlyMessageResponse {
    const admin = await this.userService.getUser({
      email: adminForgotPasswordDto.email,
      userType: 'Admin',
    });
    if (!admin) throw new NotFoundException(errorMessages.USER_NOT_REGISTERED);

    if (!admin.emailVerified) throw new BadRequestException(errorMessages.EMAIL_NOT_VERIFIED);
    ...
    const redirectUrl = `${Defaults.ADMIN_RESET_PASSWORD_URL}?token=${resetPasswordToken}`;
    await this.commonMailService
      .sendEmailWithTemplate(
        {
          name: `${admin.merchantName}`,
          email: admin.email,
          redirectUrl,
          donorProfileUrl: undefined,
        },
